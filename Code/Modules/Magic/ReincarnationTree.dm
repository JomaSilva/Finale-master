				...
				switch(alert(usr,"Pick a Yemma fruit? There is [afterlifefruitnum] fruits left.","","Yes","No")=="Yes")
					if("Yes")
						if(afterlifefruitnum>=1)
							...
							usr.contents+=A
							to_chat(view(usr), "[usr] picks fruit from the tree of life.")
						else to_chat(usr, "No fruit!")
						..()
						return
			ReincarnateTree(usr)
		..()
...
obj/Holy_Fountain
	...
	Click()
		switch(alert(usr, "A never ending stream of water flows gracefully from the holy fountain. The water is clear and untouched. Do you want to drink from the fountain?","","Yes","No"))
			if("Yes")
				to_chat(usr, "You sip from the fountain...")
				sleep(20)
				if(usr.IsAVampire==1||usr.IsAWereWolf==1||usr.Race=="Demon")
					to_chat(usr, "While it seemed like normal water at first, the after taste is repulsive. You gag a bit...")
					sleep(20)
					to_chat(usr, "You convulse and cough up blood! Drinking Holy Water is awful for someone like you!")
					usr.SpreadDamage(20,0)
				else
					to_chat(usr, "The water is very refreshing.")
					sleep(20)
					to_chat(usr, "Wait, no...to call it refreshing would be the understatement of the century! Your nutritional values are off the charts! Its like a V8 combined with every element of the food pyramid!! Your nutrition is completely restored (assuming you have a body)!")
					usr.currentNutrition = usr.maxNutrition
					return
				sleep(30)
			if("No")
				to_chat(usr, "You decide against drinking from the fountain.")