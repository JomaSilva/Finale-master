mob/proc/sayType(var/msg,var/typeversion)
	...
	switch(typeversion)
		...
		if(2)
			...
			if(Fusee && !isnamekd)
				for(var/mob/M in range(Fusee))
					M<<output("\icon[usr]<font size=[M.TextSize]>-[name] whispers something...</font>","Chatpane.Chat")
					M.to_chat_html("<font size=[M.TextSize]>-[name] whispers something...</font>", "say") //espelho HTML do teaser
			...
			for(var/mob/M in range(src))
				...
				M.to_chat_html("<font size=[M.TextSize]>-[name] whispers something...", "say") //espelho HTML do teaser (o conteudo pros ouvintes de perto ja tem chatcast abaixo)
