...
/mob/keyable/verb/Study_Other()
	...
	if(studying)
		to_chat(usr, "You stop studying your target.")
		studying = 0
		return
	...
	if(!target)
		...
		studying = 1
		to_chat(usr, "You begin to study [target]")
		while(studying)
			sleep(10)
			var/range = 0
...
/mob/keyable/verb/Focus_Skill(var/datum/skill/mind/S in learned_skills)
	set category = "Learning"
	if(!S)
		to_chat(usr, "You have nothing to focus on!")
		return
	focusskill = S.name
	to_chat(usr, "You will now focus on learning [focusskill].")

/mob/keyable/verb/Assess_Ki_Skill(mob/M in oview(20))
	set category = "Skills"
	if(usr.kiawarenessskill<=25)
		if(KiTotal()>M.KiTotal())
			to_chat(usr, "You sense that they have less ki proficiency than you.")
		else if(KiTotal()<M.KiTotal())
			to_chat(usr, "You sense that they have greater ki proficiency than you.")
		else
			to_chat(usr, "You sense that they are equal to you in ki proficiency.")
	else if(usr.kiawarenessskill<=50)
		to_chat(usr, "You assess their ki proficiencies compared to yours...")
		if(usr.kiawarenessskill>=M.kiawarenessskill)
			to_chat(usr, "You are more aware of ki than they are.")
		else
			to_chat(usr, "You are less aware of ki than they are.")
		if(usr.kieffusionskill>=M.kieffusionskill)
			to_chat(usr, "You effuse more ki than they do.")
		else
			to_chat(usr, "You effuse less ki than they do.")
		if(usr.kicirculationskill>=M.kicirculationskill)
			to_chat(usr, "You circulate more ki than they do.")
		else
			to_chat(usr, "You circulate less ki than they do.")
		if(usr.kicontrolskill>=M.kicontrolskill)
			to_chat(usr, "You have better control over ki than them.")
		else
			to_chat(usr, "You have worse control over ki than them.")
		if(usr.kiefficiencyskill>=M.kiefficiencyskill)
			to_chat(usr, "You are more efficient with ki than them.")
		else
			to_chat(usr, "You are less efficient with ki than them.")
		if(usr.kigatheringskill>=M.kigatheringskill)
			to_chat(usr, "You gather more ki than them.")
		else
			to_chat(usr, "You gather less ki than them.")
	else if(usr.kiawarenessskill>50)
		to_chat(usr, "You sense their ki skills are as follows:")
		to_chat(usr, "Awareness: [M.kiawarenessskill]")
		to_chat(usr, "Effusion: [M.kieffusionskill]")
		to_chat(usr, "Circulation: [M.kicirculationskill]")
		to_chat(usr, "Control: [M.kicontrolskill]")
		to_chat(usr, "Efficiency: [M.kiefficiencyskill]")
		to_chat(usr, "Gathering: [M.kigatheringskill]")
	...
	if(!usr.writing)
		...
		if(!A)
			...
			for(var/datum/skill/mind/nA in usr.learned_skills)
				if(A == nA.name)
					if(nA.level == 0)
						to_chat(usr, "You have no knowledge to teach!")
						return
					usr.writing=1
					...
					to_chat(usr, "You begin to write a book on [nA.name]. It will take [usr.writetarget/10] seconds to complete. Meditate to write.")
					break
	else
...
/obj/items/book/Skillbook
	...
	verb/Description()
		set category = null
		to_chat(usr, "Read to learn about ki techniques. You must know the relevant skill and be at or below the listed level.")
	verb/Study_Book()
		set category = null
		var/count = 0
		for(var/datum/skill/mind/S in usr.learned_skills)
			if(S.name == skillname)
				if(S.level<=level)
					...
					S.exp+=gain
					to_chat(usr, "Your [skillname] skill gained [gain] experience!")
					del(src)
				else
					to_chat(usr, "You can't learn anything from this book")
				count++
				break
		if(!count)
			to_chat(usr, "You don't know this skill!")
			return
