...
mob/proc/GlobalStats()
	...
	while(src)
		...
		if(ui_learned) ui_instinct_tick() //Ultra Instinct: dreno/recuperacao de precisao + maestria por uso em combate (UltraInstinct.dm)
		if(ue_learned || ue_active) ue_tick() //Ultra Ego: dreno/recuperacao de energia + Unbound Ego (ue_active sem ue_learned = kit emprestado do GoD do Instinto) (UltraEgo.dm)
