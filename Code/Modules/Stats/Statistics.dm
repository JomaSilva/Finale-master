...
mob/proc
	...
	StatTransformations()
		...
		if(godki && godki.awakened)
			shown = 1
			stat("God Ki","Maestria [round(godki.mastery, 0.1)]% - power x[godki.godki_mult]")
		if(!shown)
	...
	StatKi()
		if(("masterylevels" in tabson))
			...
			if(godki && (godki.energy || godki.awakened))
				stat("")
				stat("=God Ki Stats=")
				...
				stat("God Ki Mult: [godki.godki_mult]","Maestria: [round(godki.mastery, 0.1)]%")
