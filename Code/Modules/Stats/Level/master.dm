...
mob/proc/statify()
	...
	kicapacity = 1.3*kicapacityMod*max((MaxKi*max(log(8,max(max((kicirculationskill/10)*(kigatheringskill/10),1)*KiMod*hdnptlmod*max(Ekiskill,0.1),1)),1))*OozaruBuff,1.3*MaxKi) //wrap the WHOLE log arg in max(...,1): the inner max only floored the skill factor, so a 0 KiMod/hdnptlmod (collapsed stats) made the product 0 -> log(8,0) "not computable" killed statify every tick
	powerupcap = max((1.4*max(log(7,max(max((kicirculationskill/10)*(kicontrolskill/10),1)*KiMod*kicapacityMod*hdnptlmod*max(Ekiskill,0.1),1)),1))*OozaruBuff,1.4) //makes power capacity n shit dynamic. (same log(x,0) guard as kicapacity above)
