...
mob/Admin3/verb
	Cap_Settings()
		...
		switch(input(usr,"What setting to change?","Cap Settings","Cancel") in list("Cancel","Cap Type","Hardcap Amount","Cap Multiplier","Gains Rate","Edit Cap","Cap Catchup","Timecap Rate","Relative BP Caps"))
			if("Cap Type")
				if(CapType == 1) to_chat(usr, "World BP Cap is a Hardcap (1).")
				if(CapType == 2) to_chat(usr, "World BP Cap is a Softcap (2).")
				if(CapType == 2.5) to_chat(usr, "World BP Cap is a Softcap (2) w/o Skew.")
				if(CapType == 3) to_chat(usr, "World BP Cap is a Timecap (3).")
				...
				if(multiplier == 2)
					CapType = 2
					to_chat(world, "<b><font color=yellow>World BP Cap has been changed to a Softcap.")
				else if(multiplier == 2.5)
					CapType = 2.5
					to_chat(world, "<b><font color=yellow>World BP Cap has been changed to a Softcap without skew.")
				else if(multiplier == 1)
					CapType = 1
					to_chat(world, "<b><font color=yellow>World BP Cap has been changed to a Hardcap.")
				else
					CapType = 3
					to_chat(world, "<b><font color=yellow>World BP Cap has been changed to a Timecap.")
			if("Hardcap Amount")
				to_chat(usr, "Hardcap is at [HardCap].")
				var/multiplier=input(usr,"Enter a number for the hardcap. (1 = 1 BP)","",HardCap) as num
				HardCap=multiplier
				to_chat(world, "<b><font color=yellow>World BP Hardcap has been changed to [HardCap]")
				WriteToLog("admin","[usr]([key]) changed global BP cap to [HardCap] at [time2text(world.realtime,"Day DD hh:mm")]")
			if("Cap Multiplier")
				to_chat(usr, "Cap multiplier is at [CapRate].")
				var/multiplier=input(usr,"Enter a number for the cap-mult.","",CapRate) as num
				CapRate=multiplier
				to_chat(world, "<b><font color=yellow>World BP Cap Rate has been changed to [CapRate]")
				WriteToLog("admin","[usr]([key]) changed Cap Rate to [CapRate] at [time2text(world.realtime,"Day DD hh:mm")]")
			if("Gains Rate")
				to_chat(usr, "Gains rate is at [GainsRate].")
				...
				to_chat(world, "<b><font color=yellow>World BP Gains Rate has been changed to [GainsRate]")
				WriteToLog("admin","[usr]([key]) changed Gains Rate to [GainsRate] at [time2text(world.realtime,"Day DD hh:mm")]")
			if("Edit Cap")
				to_chat(usr, "BP Softcap is [BPCap]")
			...
			if("Timecap Rate")
				to_chat(usr, "Timecap rate is [timecaprate]")
			...
			if("Relative BP Caps")
				to_chat(usr, "Relative BP Cap rate is [relcaprate]")
