	...
	Shop_Box
		...
		verb
			Bolt()
				...
				set src in view(1)
				if(usr.ckey!=owner)
					to_chat(usr, "This isn't yours!")
					return
				var/turf/T = locate(src.x,src.y,src.z)
				if(!Bolted&&T.proprietor!=usr.ckey)
					to_chat(usr, "You can only bolt this to your own property!")
					return
				else if(!Bolted)
					to_chat(usr, "You bolt the [name].")
					Bolted=1
					return
				else if(Bolted&&shoplist.len==0)
					to_chat(usr, "You unbolt the [name].")
					Bolted=0
					return
				else if(Bolted)
					to_chat(usr, "You must empty the box to unbolt it!")
					return
			...
			Ownership()
				...
				set src in usr
				if(!owner)
					to_chat(usr, "You take ownership of this [name].")
					owner = usr.ckey
				else if(owner&&owner==usr.ckey)
					to_chat(usr, "You give up ownership of the [name].")
					owner = null
				else if(owner&&owner!=usr.ckey)
					to_chat(usr, "Only the owner can change ownership.")
					return

		Click()
			if(!Bolted)
				to_chat(usr, "This must be bolted to be used!")
				return
			if(usr.ckey==owner)
				...
				switch(choice)
					...
					if("Add")
						placestart
						if(shoplist.len>=30)
							to_chat(usr, "This box has no more room!")
							return
						else
							...
							if(isnull(cost)||cost<0)
								return
							to_chat(usr, "You put [item.name] up for sale, for [cost] zenni.")
							shoplist[item] = cost
							usr.contents-=item
							goto placestart
					if("Remove")
						removestart
						if(shoplist.len==0)
							to_chat(usr, "There are no items to take!")
							return
						else
							var/obj/item = input(usr,"Which item would you like to remove?","") as null|anything in shoplist
							if(!item)
								return
							to_chat(usr, "You withdraw [item.name].")
							usr.contents+=item
							shoplist-=item
							goto removestart
					if("Money")
						if(!money)
							to_chat(usr, "There is no money to take!")
						else
							to_chat(usr, "You withdraw [money] zenni.")
							usr.zenni+=money
							money = 0
			else
				buystart
				...
				if(!purchase)
					...
					if(usr.zenni<shoplist[purchase])
						to_chat(usr, "You can't afford this...")
						goto buystart
...
obj/items/Zenni_Minter
	...
	Click()
		...
		for(var/datum/mastery/Crafting/Handicraft/L in usr.learnedmasteries)
			...
			break
		if(handilevel < 25)
			to_chat(usr, "You need a Handicraft level of 25 or higher to use this.")
			return
		bcktohre
		itemlist = list()
		...
		if(itemlist.len)
			...
			to_chat(usr, "You have successfully created [qual] zenni!")
			goto bcktohre
