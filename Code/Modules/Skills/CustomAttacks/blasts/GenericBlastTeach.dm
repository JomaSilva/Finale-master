	...
	if(alreadyknow)
		...
		if(EnoughPoints((learncost - defaultlearncost + 1),target))
			if(target.createdblastamount>=target.maxcreatedblastamount)
				to_chat(target, "Your maximum blast count is [target.maxcreatedblastamount]! You currently have [target.createdblastamount]! Delete a few old blasts.")
				return
...
mob/proc/teachBlast(var/mob/Tm)
	if(Tm.createdblastamount>=Tm.maxcreatedblastamount)
		to_chat(Tm, "Your maximum blast count is [Tm.maxcreatedblastamount]! You currently have [Tm.createdblastamount]! Delete a few old blasts.")
		return
	...
	if(isnull(choice))
		to_chat(src, "No blast.")
		to_chat(Tm, "[src] didn't give you any preset blasts.")
		return
	else 
		var/obj/skill/CustomAttacks/ki/blast/Blast/A = new/obj/skill/CustomAttacks/ki/blast/Blast
		A = choice
