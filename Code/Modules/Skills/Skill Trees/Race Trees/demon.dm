...
/datum/skill/demon/soulabsorb
	...
	common_sense = TRUE
	after_learn()
		to_chat(savant, "You can now absorb somebody's very soul!")
		assignverb(/mob/keyable/verb/Soul_Absorb)
	before_forget()
		unassignverb(/mob/keyable/verb/Soul_Absorb)
		to_chat(savant, "You've forgotten how to eat souls!?")
...
mob/keyable/verb/Soul_Absorb(mob/M in oview(1))
	...
	if(!usr.absorbing&&M.absorbable&&!usr.KO&&usr.Planet!="Sealed")
		...
		if(M.KO&&!M.dead&&M.HasSoul)
			...
			usr.Ki+=M.Ki
			to_chat(usr, "You feel the resonance of [M]'s soul, you reach into their soul-space and absorb it!")
			to_chat(M, "You are in extreme pain from [usr] taking your soul. You no longer have a soul, which can affect some things.")
			to_chat(oview(usr), "[usr]([usr.displaykey]) seems to suck the soul straight out of [M]!")
			emit_Sound('absorb.wav')
			usr.genome.add_to_stat("Lifespan",((M.DeclineAge-M.Age)/100))
		else to_chat(usr, "They must be knocked out and must not have been already absorbed in the last 5 minutes. They also must have a soul. (This can only be done once.)")
		sleep(20)
		usr.absorbing = 0
		to_chat(usr, "You cant use it on your alts.")
/datum/skill/demon/lifeabsorb
	...
	after_learn()
		to_chat(savant, "You can now feed on somebody's very soul!")
		assignverb(/mob/keyable/verb/Life_Suck)
	before_forget()
		unassignverb(/mob/keyable/verb/Life_Suck)
		to_chat(savant, "You've forgotten how to feed on souls!?")
...
mob/keyable/verb/Life_Suck(mob/M in oview(1))
	...
	if(!usr.absorbing&&M.absorbable&&!usr.KO&&usr.Planet!="Sealed")
		...
		if(M==usr)
			...
			return
		if(M.KO&&!M.dead)
			if(!M.HasSoul) to_chat(usr, "You cant seem to find a soul in this one!")
			else
				spawn M.absorbproc()
				...
				to_chat(usr, "You feel the evil within [M]'s soul as you reach in and absorb it!")
				to_chat(M, "You feel drained, as if the life has been sucked out of your body.")
				to_chat(oview(usr), "[usr]([usr.displaykey]) seems to suck the life straight out of [M]!")
				...
				sleep(3000)
				usr.absorbing=0
		else to_chat(usr, "They must be knocked out, and must be alive, and you must not have already absorbed in the last 5 minutes.")
		usr.absorbing=0
	else to_chat(usr, "You must be not absorbing, the other person must not have been absorbed.")
...
/datum/skill/demon/Majin/after_learn()
	to_chat(savant, "A dark, devilish power courses through you...")
	assignverb(/mob/keyable/verb/Majin)
/datum/skill/demon/Majin/before_forget()
	to_chat(savant, "You breathe easy; it feels like a massive weight has been lifted from your mind as you let go of your Majin form.")
...
obj/Majinize/verb/Majinize/(mob/M in view(1))
	...
	if(M.hasmajin==1)
		...
		usr.majinized+=1
		to_chat(usr, "You have turned [usr.majinized] people into Majins, you can use this a max of <font color=red>2 times.")
		M.hasmajin=1
		M.learnSkill(new /datum/skill/demon/Majin, 0,0)
	else to_chat(usr, "You cannot turn more than 2 people into Majins.")
...
/datum/skill/demon/Devil_Bringer/after_learn()
	...
	if(savant.Race=="Demon")
		to_chat(savant, "Why don't we cause some mayhem in the mortal realm, eh?")
	else
		to_chat(savant, "Demonic energy merges with your own as you learn to manipulate space as you please.")
	assignverb(/mob/keyable/verb/Devil_Bringer)
/datum/skill/demon/Devil_Bringer/before_forget()
	if(!teacher)
		to_chat(savant, "You breathe easy as your devilish hold over space dissipates.")
	else
