			...
			if(target in view(2))
				...
				if(target in view(1))
					...
					if(doAttack(target,null,null,null,"punches",1))
						...
						to_chat(view(src), "<font color=red>[src] throws [target] forward!</font>")
		...
		if(!usr.med&&!usr.train&&!usr.KO&&usr.Ki>=kireq&&!usr.basicCD&&usr.grabbee)
			...
			if(grabMode==2)
				...
				if(grabbee in view(1))
					...
					stagger += 1
					to_chat(src, "You failed because you missed the enemy. You're stunned!")
					sleep(4)
					stagger -= 1
			sleep(15)
			attacking = 0
	...
	Power_Drag() //Drags a person across the ground dealing damage. Requires a 2 grab
		...
		if(!usr.med&&!usr.train&&!usr.KO&&usr.Ki>=kireq&&!usr.basicCD&&usr.grabbee)
			...
			if(grabMode==2)
				...
				if(!grabbee)
					stagger += 1
					to_chat(src, "You fell because you lost the enemy. You're stunned!")
					sleep(4)
					stagger -= 1
				grabbee.stagger-=1
