mob/Admin1/verb
	...
	Reward(mob/M in world)
		...
		if(usr.Admin>=2) rewardList.Add("SSJ", "SSJ2", "SSJ3", "SSJ4","SSJ Grade 2","SSJ Grade 3","Full Power","True Full Power","Super Namek","Heal","Unseal")
		...
		switch(input("Give RP Bonus", "", text) in rewardList)
			...
			if("SSJ Grade 2")
				WriteToLog("admin","[usr]([key]) made [M.name]([M.key]) SSJ Grade 2 at [time2text(world.realtime,"Day DD hh:mm")]")
				M.Ultra_SSj(2)
			if("SSJ Grade 3")
				WriteToLog("admin","[usr]([key]) made [M.name]([M.key]) SSJ Grade 3 at [time2text(world.realtime,"Day DD hh:mm")]")
				M.Ultra_SSj(3)
			if("Full Power")
