var/gameversion="0.9.6"
world
	visibility = 1 //determines if this shows up on the hub pager.
	icon_size = 32
	fps = 12 //Okay, we don't need 60 FPS.
	turf=/turf/Other/Blank
	area=/area/Outside
	version=31
	cache_lifespan=0
	loop_checks=1
	name="Dragonball Jandirus"
//status="<font color=#000000><b><font size=1>" Hate old formatting. hate it so much.
	hub = "Kingzombiethe1st.DragonballJandirus" //determines if this even shows up in the same hub. (change to format yourname.hubname, make sure if you change to set a hub at byond.com)
var/server_desc = "Godki!"

world
	New()
		..()
		log = file("DEBUG.log")
		world.status="<font color=#000000 size=1><b> Dragonball Jandirus: Hosting: [world.host], Main Fork version [gameversion] | [server_desc]</b></font>"
		spawn(10) Initialize()
		spawn(15) pspace_startup() //gera TODOS os planetas conhecidos antes de liberar o login (ProceduralSpace.dm)
		spawn(50) AutoRevive()
/*
		spawn pixelproccess()
*/

mob/Admin3/verb/World_FPS()
	set category = "Admin"
	world.fps = input(usr,"Set the world FPS. Default is 12.","",world.fps) as num

obj/DBVTitle //titulo do LOBBY (mapa z30): dbv_lobby.png = Images/dbv.png reescalado pra 480x275 (o original e 1658x949, estouraria a tela)
	icon = 'Images/dbv_lobby.png'
	pixel_x = -115 //centraliza sobre a pegada do titulo antigo (250x200): desloca metade da diferenca
	pixel_y = -37
	Savable=0

proc/SaveWorld()
	to_chat(world, "<font color=red><b><font size=3>Saving and Processing all Files")
	SaveAdmins()
	SaveYear()
	Save_Gains()
	Save_Rank()
	Save_Illegal()
	SaveItems()
	MapSave()
	AreaSave()
	Save_Settings()
	SaveMobs()
	sleep(10)
	to_chat(world, "<b><font color=yellow>Processing Complete.")

var/tmp/worldloading=0

proc/Initialize()
	set waitfor = 0
	set background = 1
	worldloading = 1
	to_chat(world, "Initializing all files...")
	firstcleaner=1
	generateTXT()
	LoadYear()
	sleep(1)
	to_chat(world, "Loaded Years")
	LoadAdmins()
	sleep(1)
	to_chat(world, "Loaded (b)Admins")
	Load_Ban()
	sleep(1)
	to_chat(world, "Loaded Idio- I mean Bans")
	Load_Illegal()
	sleep(1)
	to_chat(world, "Loaded Illegals")
	Load_Gains()
	sleep(1)
	to_chat(world, "Loaded Muh Gains")
	LoadStory()
	sleep(1)
	to_chat(world, "Loaded Lol Story")
	LoadRules()
	sleep(1)
	to_chat(world, "Loaded Admin Excuses")
	Load_Rank()
	sleep(1)
	to_chat(world, "Loaded Skill Hoarders")
	Init_Masteries()
	sleep(1)
	to_chat(world, "Loaded Masteries")
	LoadIntro()
	sleep(1)
	to_chat(world, "Loaded The Introduction")
	Load_Settings()
	sleep(1)
	to_chat(world, "Loaded the optimized settings for maxmimum gaming potential")
	Init_Genome()
	sleep(1)
	to_chat(world, "Initialized Genomes")
	LoadItems()
	Init_Alchemy()
	sleep(1)
	to_chat(world, "YOU CANNOT HANDLE MY POTIONS. Potions loaded.")
	Init_Recipes()
	sleep(1)
	to_chat(world, "It's a piece of cake to bake a pretty cake, WHAT, (If the way is hazy!) Recipes loaded.")
	MapLoad()
	sleep(1)
	to_chat(world, "Nah Final Destination was cool. Map loaded.")
	AreaLoad()
	sleep(1)
	to_chat(world, "Loading ATMOSPHERE")
	Build_Vegeta_Structures()
	sleep(1)
	to_chat(world, "The cities of Vegeta rise from the red soil.")
	Build_Sky_NPCs()
	sleep(1)
	to_chat(world, "King Kai and Enma Daioh watch over the Other World.")
	Build_Planet_Population()
	sleep(1)
	to_chat(world, "The peoples of Vegeta, Earth and Namek stir to life.")
	LoadMobs()
	sleep(1)
	Cleaner()
	sleep(1)
	to_chat(world, "Cleaning lady on board.")
	WorldTime()
	sleep(1)
	to_chat(world, "I just put the 'Morning' in Morning-Wood. World Timer loaded.")
	spawn(10)
		WorldClock()
		WorldSubClocks()
	spawn World_Ticker()
	to_chat(world, "Time to charge the clock, you cock.")
	spawn(10) Years()
	to_chat(world, "Old... age... kicking... in...")
	spawn(10) Dungeon_Timer()
	to_chat(world, "Dungeons rolling up!")
	spawn(10) Boss_Events_Init()
	to_chat(world, "The universe holds its breath... Boss events armed.")
	spawn(10) Planet_Rep_Init()
	to_chat(world, "The peoples remember their heroes and their butchers.")
	spawn(10) Conq_Init()
	to_chat(world, "Banners of conquest ripple across the stars.")
	spawn(10) RankQuests_Init()
	to_chat(world, "The thrones of the universe demand their dues.")
	spawn(10) Build_Eternal_Dragonballs()
	to_chat(world, "Seven stars sleep beneath the green sky of Namek.")
	spawn Restart_Handler()
	to_chat(world, "All files loaded.")
	spawn jokecheck()
	worldloading = 0
	spawn generate_spells()

proc/World_Ticker() //Use this for your shit that doesn't need to be resource intensive or before anything.
	set waitfor = 0
	set background = 1
	CHECK_TICK
	if(TimeStopped)
		TimeStopDuration-=1
		if(TimeStopDuration<=0)
			TimeStopped = 0
			TimeStopDuration=0
	sleep(1)
	CHECK_TICK
	spawn World_Ticker()

proc/Restart_Handler()
	set waitfor = 0
	sleep(324000)
	Restart()


//have been having issues with save files not deleting defunct shit? use this.
/datum
	var/plsdelete=0
	/*Body/Head/Brain/plsdelete = 1 example*/
	New()
		..()
		if(plsdelete) del(src)

/atom
	New()
		..()
		/*if(lightme)
			light = new(src, lightradius, lightintensity)*/
		if(plsdelete) del(src)

var/aprilfoolson = 0

proc/jokecheck()
	var/monthcheck = time2text(world.realtime,"MM")
	var/daycheck = time2text(world.realtime,"DD")
	if(monthcheck == "04" && daycheck == "01")
		aprilfoolson = 1
		to_chat(world, "<font color=red size=3>SELF DESTRUCT SEQUENCE INITIATED")
		sleep(10)
		to_chat(world, "<font color=red size=3>TIME TO DESTRUCTION: 5")
		sleep(10)
		to_chat(world, "<font color=red size=3>TIME TO DESTRUCTION: 4")
		sleep(10)
		to_chat(world, "<font color=red size=3>TIME TO DESTRUCTION: 3")
		sleep(10)
		to_chat(world, "<font color=red size=3>TIME TO DESTRUCTION: 2")
		sleep(10)
		to_chat(world, "<font color=red size=3>TIME TO DESTRUCTION: 1")
		sleep(10)
		to_chat(world, "<font color=white size=3>Shaggy:</font><font color=purple size=4> Hakai.</font>")
		sleep(15)
		to_chat(world, "...")
		sleep(30)
		to_chat(world, "APRIL FOOLS! Super Saiyan 5 is available for saiyans today only. For the Admin's sake, don't abuse this please.")