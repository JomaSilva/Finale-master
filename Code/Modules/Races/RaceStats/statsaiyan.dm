...
mob/var/futureSSJStage = 0 //LEGADO (modelo antigo de estagios 0-10): mantido so para MIGRAR saves antigos -> futuressjmastery
mob/var/futureSSJExp = 0 //LEGADO: progresso antigo em ticks; entra como fracao na migracao
mob/var/futuressjmastery = 0 //Future Lineage: maestria % (0-100) do Future Super Saiyan. Cada 10% = +2x de poder (2x -> 20x); sobe MUITO devagar usando a forma (config em supersaiyan.dm)
mob/var/futureSSJMigrated = 0 //ja converteu estagio/exp antigos para a maestria % (roda 1x por personagem)
mob/proc/statsaiyan()
	NoAscension = 1
