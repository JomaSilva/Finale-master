...
mob/proc/AddHair()
	...
	if(Class == "Legendary Primal Saiyan" && ssj) //Primal Legendary: LSSJ/LSSJ2/LSSJ3 = cabelo SSJ1/2/3 verde; C-Type (ssj1) normal
		switch(ssj)
			if(1) updateOverlay(/obj/overlay/hairs/ssj/ssj1)
			if(2) updateOverlay(/obj/overlay/hairs/ssj/ssj1,ssjhair,0,100,0)
			if(3) updateOverlay(/obj/overlay/hairs/ssj/ssj2,ssjhair,0,100,0)
			if(3.5) updateOverlay(/obj/overlay/hairs/ssj/ssj3,ssjhair,0,100,0)
			if(4) updateOverlay(/obj/overlay/hairs/ssj/ssj4)
			if(5) updateOverlay(/obj/overlay/hairs/ssj/ssj4)
			if(6) updateOverlay(/obj/overlay/hairs/ssj/ssj4)
		return
	switch(ssj)
		if(1)
			if(ssjdrain<=0.010)
